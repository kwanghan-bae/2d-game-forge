...
사용자의 변경을 보존한다. `games/inflation-rpg/src/systems/`의 `paradoxSpiral.ts`, `paradoxSpiral.test.ts`, `paradoxSpiralBalance.test.ts`와 사용자 `output/`, `tmp/`는 변경·커밋하지 않는다. 현재 제품은 출시 전이므로 외부 저장 호환 계약을 유지하지 않는다.

보고와 제품 문서는 한국어로 작성한다. 구현 완료는 대상 변경의 검증 증거로 판단한다. 현재 게임의 package·route·Capacitor `appId`·canonical 저장 키를 제품 식별자의 단일 기준으로 삼고, 이전 식별자나 저장 키를 실행 계약으로 추가하지 않는다.
