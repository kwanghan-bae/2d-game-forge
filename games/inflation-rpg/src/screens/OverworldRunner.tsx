...
import { getOverkillMessage, getDangerZoneMessage, getCloseCallMessage, getCriticalHitMessage, getBossRageMessage, getEliteMessage, getVillageRestMessage, getFirstBloodMessage, getRevengeKillMessage, getLuckyDodgeMessage, getMercyMessage, getPerkReviveMessage, getBossPhaseShiftMessage } from '../data/battleFlavorText';
    ...
    bootPhaser(
      ...
      (event) => {
        if (event.type === 'arrived_at') {
          ...
          if (gambitEv) setGambitResult({ accepted: gambitEv.accepted, hpCost: gambitEv.hpCost, goldReward: gambitEv.goldReward });
          ...
          const flavor =
           ...
           evs.some(e => e.type === 'event_treasure_shrine') ? '✨ 보물 제단 발견!' :
            ...
            evs.some(e => e.type === 'boss_phase_shift') ? getBossPhaseShiftMessage(tick) :
          ...
          for (const ev of evs) {
           ...
           if (ev.type === 'vc_survival_burst') {
            ...
            if (ev.type === 'boss_phase_shift') {
              setSpectacleQueue(q => [...q, { id: `bps-${Date.now()}`, kind: 'boss_phase_shift', enrageAtkMul: ev.enrageAtkMul }]);
            }
          }
