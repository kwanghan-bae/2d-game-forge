...
const MAX_OFFLINE_DISPLAY_SECONDS = Village_OFFLINE_CAP_MS / 1000;
...
export function OfflineResultScreen({ summary, pendingExpeditionConfirmation = false, onClose, onOpenExpedition, onDoubleReward, canDoubleReward = true, doubleRewardPending = false, adsToday = 0, adFree = false }: Props) {
  ...
  return (
    <div className="village-overlay" role="dialog" aria-modal="true" aria-labelledby="village-offline-result-title">
      <section ref={dialogRef} className="village-modal" data-testid="village-offline-result">
        <div className="village-kicker">돌아온 후원자</div>
        <h2 id="village-offline-result-title">마을이 당신을 기다렸습니다</h2>
        ...
        {summary.wasClamped && <div className="village-alert">오프라인 보상은 최대 8시간까지만 계산했습니다.</div>}
        {summary.clockAnomaly === 'backwards' && <div className="village-alert">기기 시간이 이전 처리 시각보다 빠릅니다. 중복 보상을 막았습니다.</div>}
        {summary.clockAnomaly === 'future' && <div className="village-alert">저장 시각이 현재보다 미래입니다. 기기 시간을 확인해 주세요.</div>}
        {summary.clockAnomaly === 'invalid' && <div className="village-alert">기기 시각을 확인할 수 없어 보상을 정산하지 않았습니다.</div>}
        {summary.clockAnomaly === null && notes
          .filter((note) => typeof note === 'string' && note.trim().length > 0)
          .map((note, index) => <div className="village-alert" key={`${note}-${index}`}>{note}</div>)}
        <div className="village-detail-grid">
          {resourceEntries.length > 0
            ? resourceEntries.map(([key, value]) => <div className="village-detail-stat" key={key}><small>{getVillageCurrencyName(key)}</small><strong>+{value.toLocaleString('ko-KR')}</strong></div>)
            : <div className="village-detail-stat"><small>정산</small><strong>획득 재화 없음</strong></div>}
        </div>
        {equipmentGained.length > 0 && <div className="village-alert">장비 획득 · {equipmentGained.map(getVillageEquipmentName).join(', ')}</div>}
        {equipmentUpgraded.length > 0 && <div className="village-alert">장비 강화 · {equipmentUpgraded.map(getVillageEquipmentName).join(', ')}</div>}
        <p>{completedTaskIds.length}개 작업 완료 · {summary.completedExpedition
          ? '원정 귀환 완료 · 원정 화면에서 결과를 확인하세요.'
            ...
            : '선택형 사건은 보류됨'}</p>
        {onDoubleReward && <button type="button" className="village-btn village-btn--quiet" disabled={!hasPositiveResourceReward || !canDoubleReward || doubleRewardPending || (!adFree && adsToday >= Village_DAILY_REWARDED_LIMIT)} onClick={onDoubleReward}>
          {doubleRewardPending ? '오프라인 보상 처리 중' : !hasPositiveResourceReward ? '이번 정산은 2배 대상 없음' : !canDoubleReward ? '보상 2배 적용 완료' : !adFree && adsToday >= Village_DAILY_REWARDED_LIMIT ? '오늘 광고 한도 도달' : adFree ? '광고 제거 적용 · 오프라인 재화 2배' : '광고 보고 오프라인 재화 2배'}
        </button>}
        {onOpenExpedition && (summary.completedExpedition || pendingExpeditionConfirmation) && <button type="button" className="village-btn village-btn--quiet" onClick={() => { onOpenExpedition(); onClose(); }}>
          원정 결과 보기
        </button>}
        <button ref={closeButtonRef} type="button" className="village-btn village-btn--primary" onClick={onClose}>마을 확인</button>
      </section>
    </div>
  );
}
