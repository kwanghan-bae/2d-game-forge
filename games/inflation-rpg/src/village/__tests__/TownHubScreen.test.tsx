...
function renderHub(overrides: Partial<React.ComponentProps<typeof TownHubScreen>> = {}) {
  const save = overrides.save ?? createInitialVillageSave(95);
...
describe('Village town hub support assignment', () => {
  ...
  it('keeps a facility startable without its specialist when that agent is busy', () => {
    const save = createInitialVillageSave(96);
  ...
  it('disables agent rest when the persisted fatigue is malformed', () => {
    ...
    const agent = screen.getByText('대장장이').closest('.village-agent') as HTMLElement | null;
  ...
  it('surfaces the closest objective beside the hero before the facility list', () => {
    ...
    const objective = screen.getByTestId('village-top-objective');
  ...
  it('groups the overview and management areas for the responsive town layout', () => {
    ...
    const overview = within(town).getByTestId('village-town-overview');
    const management = within(town).getByTestId('village-town-management');

    expect(overview).toContainElement(screen.getByTestId('village-town-hero'));
    expect(overview).toContainElement(screen.getByTestId('village-top-objective'));
  ...
  it('renders the Joseon warrior sheet as the memorable town hero portrait', () => {
    ...
    const sprite = screen.getByTestId('village-hero-sprite');
    expect(sprite).toHaveClass('village-hero-sprite');
    expect(sprite).toHaveAttribute('aria-hidden', 'true');
  });

  it('moves focus to the hero heading when the town opens', () => {
    const save = createInitialVillageSave(106);
    renderHub({ save });
  ...
  it('prioritizes an awaiting expedition confirmation in the closest objective', () => {
    const save = createInitialVillageSave(107);
    save.run.expedition = {
  ...
  it('keeps the hub readable when a malformed result has an unknown Realm id', () => {
    const save = createInitialVillageSave(103);
    save.run.lastExpeditionResult = {
  ...
  it('normalizes malformed hero and agent numbers before rendering the hub', () => {
    const save = createInitialVillageSave(105);
  ...
  it('does not expose an infinite remaining time for malformed facility clocks', () => {
    const initial = createInitialVillageSave(99);
  ...
  it('disables intervention charging when the safety reserve is already full', () => {
    const save = createInitialVillageSave(100);
  ...
  it('disables the ad-free purchase button while the store request is pending', () => {
    const save = createInitialVillageSave(109);
  ...
  it('shows the instant-task provider state and blocks task mutations while pending', () => {
    const initial = createInitialVillageSave(110);
    const started = startFacilityTask(initial, 'temple', initial.createdAt);
    expect(started.ok).toBe(true);
    if (!started.ok) return;
  ...
  it('disables instant facility completion after the daily ad limit', () => {
    ...
    renderHub({ save: started.save, monetizationAvailable: true, adsToday: Village_DAILY_REWARDED_LIMIT, onInstantTask: vi.fn() });
