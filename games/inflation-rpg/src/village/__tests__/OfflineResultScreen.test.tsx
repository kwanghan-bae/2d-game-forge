...
describe('Village offline result screen', () => {
  ...
  it('explains whether the device clock moved backwards or into the future', () => {
    ...
    expect(screen.getByTestId('village-offline-result')).toHaveTextContent('이전 처리 시각보다 빠릅니다');

    rerender(<OfflineResultScreen summary={summary({ clockAnomaly: 'future' })} onClose={() => {}} />);
    expect(screen.getByTestId('village-offline-result')).toHaveTextContent('저장 시각이 현재보다 미래입니다');

    rerender(<OfflineResultScreen summary={summary({ clockAnomaly: 'invalid' })} onClose={() => {}} />);
    expect(screen.getByTestId('village-offline-result')).toHaveTextContent('기기 시각을 확인할 수 없어');
  });
  ...
  it('makes an empty offline reward explicit', () => {
    ...
    expect(screen.getByTestId('village-offline-result')).toHaveTextContent('획득 재화 없음');
  ...
  it('calls out a risky expedition that still needs player confirmation', () => {
    ...
    expect(screen.getByTestId('village-offline-result')).toHaveTextContent('위험 원정 결과 확인 필요');
    expect(screen.getByTestId('village-offline-result')).toHaveTextContent('원정 화면에서 결과를 확인하세요');
    expect(screen.getByTestId('village-offline-result').textContent).not.toContain('보스 결과를 확인하세요');
  });
  ...
  it('keeps a completed expedition summary neutral until its outcome is shown', () => {
    ...
    const result = screen.getByTestId('village-offline-result');
  ...
  it('does not expose an unknown resource storage key to players', () => {
    ...
    expect(screen.getByTestId('village-offline-result')).toHaveTextContent('기타 재화');
    expect(screen.getByTestId('village-offline-result').textContent).not.toContain('unknown');
  });
  ...
  it('shows offline settlement notes that explain the applied boundary', () => {
    ...
    expect(screen.getByTestId('village-offline-result')).toHaveTextContent('안전한 작업만 오프라인으로 정산했습니다.');
  });
  ...
  it('closes from Escape and exposes the dialog title to assistive technology', () => {
    ...
    expect(dialog).toHaveAttribute('aria-labelledby', 'village-offline-result-title');
    expect(screen.getByRole('heading', { name: '마을이 당신을 기다렸습니다' })).toHaveAttribute('id', 'village-offline-result-title');
