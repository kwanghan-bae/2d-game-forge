import type { FacilityId, RealmEncounterDefinition, RealmId, SupportAgentId, VillageCurrencyKey, VillagePolicy } from './types';

export const CURRENCY_LABELS: Record<VillageCurrencyKey, string> = {
  spirit: '신력',
  gold: '금화',
  materials: '재료',
  rift: '균열석',
...
export function getVillageCurrencyName(key: string): string {
  return Object.prototype.hasOwnProperty.call(CURRENCY_LABELS, key)
    ? CURRENCY_LABELS[key as VillageCurrencyKey]
    : '기타 재화';
}
...
export interface RealmDefinition {
  ...
  cost: Partial<Record<VillageCurrencyKey, number>>;
...
export function getVillageFacilityDefinition(id: string): FacilityDefinition | undefined {
  return getOwnDefinition(FACILITY_DEFINITIONS, id);
}

export function getVillageFacilityName(id: string): string {
  return getVillageFacilityDefinition(id)?.nameKR ?? '기록되지 않은 시설';
}

export function getVillageAgentDefinition(id: string): AgentDefinition | undefined {
  return getOwnDefinition(AGENT_DEFINITIONS, id);
}

export function getVillageRealmDefinition(id: string): RealmDefinition | undefined {
  return getOwnDefinition(REALM_DEFINITIONS, id);
}

export function getVillageRealmName(id: string): string {
  return getVillageRealmDefinition(id)?.nameKR ?? '기록되지 않은 영역';
}
...
export const POLICY_LABELS: Record<VillagePolicy, string> = {
  aggression: '공격 우선',
  hoarding: '안전 비축',
  training: '성장 집중',
};

export function getVillagePolicyName(policy: string): string {
  return Object.prototype.hasOwnProperty.call(POLICY_LABELS, policy)
    ? POLICY_LABELS[policy as VillagePolicy]
    : '정책 확인 필요';
}
