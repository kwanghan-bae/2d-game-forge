...
describe('HudIndicatorBar', () => {
  ...
  it('shows event badges for active trial/colosseum/voidRift', () => {
    const result = buildHudIndicators({
      ...
      influencingTraits: [],
      inspirationRemaining: 0,
      activeEvents: { trialGroundsRemaining: 2, colosseumRemaining: 0, voidRiftRemaining: 1, stormNexusRemaining: 0, rainSanctuaryRemaining: 0, fogAmbushRemaining: 0, windGaleRemaining: 0, snowDriftRemaining: 0 },
    });
    expect(result).toHaveLength(2);
  ...
  it('shows no event badges when all events are zero', () => {
    const result = buildHudIndicators({
      ...
      influencingTraits: [],
      inspirationRemaining: 0,
      activeEvents: { trialGroundsRemaining: 0, colosseumRemaining: 0, voidRiftRemaining: 0, stormNexusRemaining: 0, rainSanctuaryRemaining: 0, fogAmbushRemaining: 0, windGaleRemaining: 0, snowDriftRemaining: 0 },
    });
