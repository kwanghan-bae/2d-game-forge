...
import type { FacilityId, V4Policy, V4SaveEnvelope } from '../types';
...
function persistAndReload(save: V4SaveEnvelope, storage: Storage): V4SaveEnvelope {
  expect(persistV4Save(save, storage)).toBe(true);
  const loaded = loadV4Save(storage);
  expect(loaded).not.toBeNull();
  return loaded ?? save;
}

function runRandomizedSaveSequence(seed: number): V4SaveEnvelope {
  const storage = memoryStorage();
  let randomState = seed;
  let now = Date.now();
  let save = createInitialV4Save(seed);
  now = save.updatedAt;
  ...
  for (let step = 0; step < 80; step += 1) {
    ...
    const policy: V4Policy = random.value < 0.34
      ? 'aggression'
      : random.value < 0.67
        ? 'hoarding'
        : 'training';
    ...
    switch (step % 8) {
      case 0:
        save = setV4Policy(save, policy, now);
        break;
      case 1:
        save = updateV4Settings(save, {
          music: random.value,
          sfx: 1 - random.value,
          muted: step % 2 === 0,
        }, now);
...
describe('V4 save state-machine invariants', () => {
