...
export class EncounterEngine {
  ...
  resolveEncounter(hero: HeroEntity, kind: LandmarkKind, landmarkId: string): OverworldEvent[] {
    ...
    if (kind === 'enemy' || kind === 'boss') {
      ...
      const provingMul = this.provingGroundsExpRemaining > 0 ? PROVING_GROUNDS_REWARD_EXP_MUL : 1;
      if (this.provingGroundsExpRemaining > 0) this.provingGroundsExpRemaining--;
