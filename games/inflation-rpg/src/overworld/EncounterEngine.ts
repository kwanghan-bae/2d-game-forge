...
// C461: prestige shield recharge — prestige recharges all shields
...
// C463: combo exp velocity — exp scales with combo speed (consecutive fights)
...
// C467: death ATK surge — dying grants temp ATK surge
...
// C472: combo shield regen boost — high combo regenerates shields faster
// (uses existing COMBO_SHIELD_REGEN_THRESHOLD)
...
// C475: wave ATK compound — consecutive waves compound ATK bonus
...
// C479: crit gold cascade — crits cascade gold bonus
...
// --- C501-C510: Trade-off & Conditional Mechanics ---
// C501: blood pact — sacrifice HP for ATK boost
export const BLOOD_PACT_HP_COST = 0.1;
export const BLOOD_PACT_ATK_BONUS = 0.25;
export const BLOOD_PACT_THRESHOLD = 0.5; // only triggers below 50% HP
// C502: greed gambit — gold above threshold boosts exp but slows gold gain
...
// C503: adrenaline rush — low HP triggers massive ATK but disables healing
...
// C512: full HP fortune — gold ×1.5 when HP = 100%
export const FULL_HP_FORTUNE_GOLD_MUL = 1.5;
// C513: combo gate — exp burst on first reaching combo 50
...
export const GOLD_THRESHOLD_DEF_BONUS = 0.5;
// C515: death proximity crit — survive at 1 HP → next fight 100% crit
...
// C518: deep danger exp — danger ≥ 8 adds extra exp layer
export const DEEP_DANGER_THRESHOLD = 8;
export const DEEP_DANGER_EXP_MUL = 1.5;
// C519: prestige ready defiance — fighting past prestige threshold = bonus accumulate
    ...
    if (kind === 'enemy' || kind === 'boss') {
      ...
      while (eHp > 0 && !hero.staggered && hitCount < MAX_COMBAT_TURNS) {
        ...
        if (eHp > 0) {
          ...
          const incomingDmg = Math.max(1, Math.floor(rageAtk * mercyReduction * shieldReduction * goldArmorMul * nightDmgMul * armorMul * vigorMul * goldShieldMul * comboShieldMul * goldOverflowMul * bossShieldMul * prestigeDangerMasteryMul * goldThresholdDefMul));
