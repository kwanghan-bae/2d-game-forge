...
import { resolvePostCombatEvent, type PostCombatContext, type PostCombatResult, type PostCombatEventType } from './encounter/PostCombatEventResolver';
...
import { computeNight, WeatherSubsystem, type Weather } from './encounter/WeatherSystem';
...
import { RunStatistics, type RunStatisticsData } from './encounter/RunStatistics';

type PostCombatSimpleEventType = Exclude<PostCombatEventType,
...
export class EncounterEngine {
  ...
  private reputationFired = false; // C883: reputation payoff one-shot flag
  ...
  private inflationRushPending = false; // C989: waiting for player choice (ride vs cashout)
  private inflationRushChoiceResolved: 'ride' | 'cashout' | null = null; // C989
  ...
  getRunStatistics(): RunStatisticsData { return this.runStats.snapshot(); }
  resetRunStatistics(): void { this.runStats.reset(); }
  ...
  private applyEventResolve(id: EventId, accept: boolean): void {
    const effects = this.eventOrch.resolve(id, accept, {
      ...
      rngChance: (rate: number) => this.rng.float() < rate,
    });
    if (accept) {
      ...
      // C940: Endgame Surge — ATK buff + possible stat shard
      if (effects.endgameSurgeRemaining) {
        this.midGameBuffs.activate('endgame_surge', effects.endgameSurgeRemaining);
        if (effects.endgameSurgeShardGranted) {
          this.statShardAtk += STAT_SHARD_ATK_FLAT;
        }
      }
      // C941: Ascension Trial — enemy level boost + EXP boost
      if (effects.ascensionTrialRemaining) {
        this.midGameBuffs.activate('ascension_trial', effects.ascensionTrialRemaining);
      }
      // C941: Echo Memory — ATK buff
      if (effects.echoMemoryRemaining) {
        this.midGameBuffs.activate('echo_memory', effects.echoMemoryRemaining);
      }
      // C941: Shard Fusion — sacrifice shards for big permanent ATK
      if (effects.shardFusionGranted && this.statShardAtk >= effects.shardFusionCost * STAT_SHARD_ATK_FLAT) {
        this.statShardAtk -= effects.shardFusionCost * STAT_SHARD_ATK_FLAT;
        this.statShardAtk += SHARD_FUSION_ATK_REWARD;
      }
  ...
  // C890: Last Stand choice
  hasPendingLastStandChoice(): boolean { return this.lastStandPending; }
  ...
  setFirstTrialChoice(choice: 'heal' | 'atk' | 'exp'): void {
  ...
  // C921: Wandering Sage choice API
  hasPendingWanderingSageChoice(): boolean { return this.wanderingSagePending; }
  setWanderingSageChoice(choice: 'exp' | 'atk'): void {
    this.wanderingSageChoiceResolved = choice;
    this.wanderingSagePending = false;
    this.choiceHistory.record(this.getTotalFights(), 'wandering_sage', choice, classifyChoice('wandering_sage', choice));
  }

  // C926: Elder's Judgment choice API
  hasPendingEldersJudgmentChoice(): boolean { return this.eldersJudgmentPending; }
  setEldersJudgmentChoice(choice: 'double_down' | 'diversify'): void {
    this.eldersJudgmentChoiceResolved = choice;
    this.eldersJudgmentPending = false;
    this.choiceHistory.record(this.getTotalFights(), 'elders_judgment', choice, classifyChoice('elders_judgment', choice));
  }

  // C989: Inflation Rush choice API
  hasPendingInflationRushChoice(): boolean { return this.inflationRushPending; }
  setInflationRushChoice(choice: 'ride' | 'cashout'): void {
    this.inflationRushChoiceResolved = choice;
    this.inflationRushPending = false;
    this.choiceHistory.record(this.getTotalFights(), 'inflation_rush', choice, classifyChoice('inflation_rush', choice));
  }

  // C959: Veteran's Challenge accept/decline
  hasPendingVeteransChallengeChoice(): boolean { return this.choiceEngine.hasPendingVeteransChallengeChoice(); }
  ...
  resolveEncounter(hero: HeroEntity, kind: LandmarkKind, landmarkId: string): OverworldEvent[] {
    ...
    if (kind === 'enemy' || kind === 'boss') {
      ...
      // C941: Ascension Trial — stronger enemy level mul (stacks with trial_grounds if somehow both active)
      const ascensionLevel = this.midGameBuffs.isActive('ascension_trial') ? Math.floor(trialGroundsLevel * ASCENSION_TRIAL_ENEMY_LEVEL_MUL) : trialGroundsLevel;
      const effectiveEnemyLevel = Math.floor(ascensionLevel * voidRiftMul);
      ...
      // C977: capture vc_atk state before tick to detect expiry (survival burst)
      const hadVcAtk = this.midGameBuffs.isActive('vc_atk');
      // C983: capture inflation_burst state before tick (rush activation on expiry)
      const hadInflBurst = this.midGameBuffs.isActive('inflation_burst');
      ...
      // C974: Veteran's Challenge HP drain — 3% maxHP per fight while vc_atk active
      if (this.midGameBuffs.isActive('vc_atk')) {
        const vcDrain = Math.floor(hero.hpMax * VETERANS_CHALLENGE_HP_DRAIN);
        hero.hp -= vcDrain;
        if (hero.hp < 1) hero.hp = 1;
        // C978: emit progress for UI tension (current fight N out of total duration)
        const vcRemaining = this.midGameBuffs.remaining('vc_atk');
        const vcCurrent = VETERANS_CHALLENGE_DURATION - vcRemaining;
        events.push({ type: 'vc_progress', current: vcCurrent, total: VETERANS_CHALLENGE_DURATION, hpPercent: hero.hp / hero.hpMax });
        events.push({ type: 'vc_hp_drain', value: vcDrain, hpAfter: hero.hp });
      }

      // C977: Veteran's Challenge survival burst — survived all 10 fights → EXP explosion
      if (hadVcAtk && !this.midGameBuffs.isActive('vc_atk')) {
        const burst = Math.floor(hero.level * VETERANS_CHALLENGE_SURVIVAL_BURST);
        hero.gainExp(burst);
        events.push({ type: 'vc_survival_burst', value: burst });
      }
      ...
      // C983+C989: Inflation Rush — post-burst choice (ride ×2 EXP vs cashout gold)
      if (hadInflBurst && !this.midGameBuffs.isActive('inflation_burst')) {
        if (this.inflationRushChoiceResolved === 'ride') {
          // C985: healthy hero gets extended rush
        ...
        } else if (this.inflationRushChoiceResolved === 'cashout') {
          // C989: immediate gold payout
          ...
          this.inflationRushPending = true;
          events.push({ type: 'inflation_rush_pending' });
        }
      }
      ...
      const atkResult = computeAtkMultipliers(this.buildAtkContext({
        hero, isBoss, isDangerZone, isElite, landmarkId, combatBuffs, weather,
        hpBelowBloodPact, hadShieldSacrifice, hadPrestigeEcho, prestigeEchoDecay,
        eventMomentumAtkActive, inspirationActive, hadWaveExhaustion,
        hadShieldBreakBurst, lowHpFury, bloodFurySynergyActive, elderWisdomActive,
        desperateTradeActive, antiSynergyActive, goldenHourHighCombo,
        wealthSacrificeActive, comboPrestigeFlat, waveChainAtk, combatMasteryVal,
        deathCountAtk, dangerComboAtk, comboAtkMilestone, kind,
      }));
      ...
      while (eHp > 0 && !hero.staggered && hitCount < MAX_COMBAT_TURNS) {
        ...
        if (eHp > 0) {
          ...
          const totalDrMul = computeDamageReduction({
            ...
            titanArenaActive: this.midGameBuffs.isActive('titan_arena'),
            astralParadoxActive: this.midGameBuffs.isActive('astral_paradox'),
      ...
      // C331: elite fury is decremented by tickSimpleDurations at the start
      // of the next fight; do not maintain a second legacy counter here.
      // C336: decrement danger cascade
      const isOverkill = hitCount === 1 && !hero.staggered;
      // C981+C988: Inflation Burst result — meaningful overkill requires damage ≥ 3× enemy HP
      if (this.midGameBuffs.isActive('inflation_burst') && isOverkill) {
        const meaningfulOverkill = totalDamageDealt >= enemyHp * 3;
        events.push({ type: 'inflation_burst_result', overkill: meaningfulOverkill, atkMul: INFLATION_BURST_ATK_MUL });
      }
      ...
      const { multiplier: expMul, breakdown: expBreakdownResult } = computeExpMultiplierWithBreakdown({
        ...
        titanArenaActive: this.midGameBuffs.isActive('titan_arena'),
        astralParadoxActive: this.midGameBuffs.isActive('astral_paradox'),
        crimsonTitheActive: this.midGameBuffs.isActive('crimson_tithe'),
      ...
      const { cappedExp: baseExpGainPost } = computePostVictoryExp({
        baseExpGain, expMul,
      ...
      // C866: Proving Grounds EXP buff (post-multiplier)
      // C877: Manual accept gives +25% bonus
      const provingBase = this.provingGroundsExpRemaining > 0 ? PROVING_GROUNDS_REWARD_EXP_MUL : 1;
      const provingMul = provingBase * (this.provingGroundsManualAccept && this.provingGroundsExpRemaining > 0 ? (1 + PROVING_GROUNDS_MANUAL_BONUS) : 1);
      if (this.provingGroundsExpRemaining > 0) {
        this.provingGroundsExpRemaining--;
      ...
      if (didCrit) {
        events.push({ type: 'critical_hit', streak: this.comboStreak, damage: totalDamageDealt });
    ...
    return this.recordAndReturn(events, hero);
  }

  // C833: Record run statistics from resolved events before returning
  private recordAndReturn(events: OverworldEvent[], hero: HeroEntity): OverworldEvent[] {
    this.runStats.recordFight();
    this.runStats.recordLevel(hero.level);
    this.runStats.recordCombo(this.comboStreak);
    for (const e of events) {
      if (e.type === 'critical_hit') this.runStats.recordCriticalHit();
        ...
        if (e.choice === 'heal' || e.choice === 'atk') {
          this.runStats.recordMerchant(e.choice);
        }
        this.runStats.recordEvent('wandering_merchant');
      } else if (e.type === 'event_risk_gambit') {
        this.runStats.recordGambit(e.accepted, true);
        this.runStats.recordEvent('risk_gambit');
      } else if (e.type.startsWith('event_')) {
        this.runStats.recordEvent(e.type);
      }
    }
    return events;
  }
  ...
  // C849: Build ATK multiplier context from engine state + local captures
  private buildAtkContext(p: {
    hero: HeroEntity; isBoss: boolean; isDangerZone: boolean; isElite: boolean;
    landmarkId: string; combatBuffs: any;
    weather: Weather; hpBelowBloodPact: boolean; hadShieldSacrifice: boolean;
    hadPrestigeEcho: boolean; prestigeEchoDecay: number;
    eventMomentumAtkActive: boolean; inspirationActive: boolean;
    hadWaveExhaustion: boolean; hadShieldBreakBurst: boolean;
    lowHpFury: boolean; bloodFurySynergyActive: boolean;
    elderWisdomActive: boolean; desperateTradeActive: boolean;
    antiSynergyActive: boolean; goldenHourHighCombo: boolean;
    wealthSacrificeActive: boolean; comboPrestigeFlat: number;
    waveChainAtk: number; combatMasteryVal: number; deathCountAtk: number;
    dangerComboAtk: number; comboAtkMilestone: number; kind: string;
  }) {
    return {
      damping: this.opts.damping ?? 1.0,
      bossAtkMul: p.isBoss ? (this.opts.getBossIntroAtkMul?.() ?? 1.0) : 1.0,
      ...
      isRevengeFight: this.lastDeathEnemyId === p.landmarkId,
      killMilestones: this.killMilestones,
      hpRatio: p.hero.hp / p.hero.hpMax,
      fightsSinceVillage: this.fightsSinceVillage,
      shrineTithes: this.shrineTithes,
      hadShieldBreak: p.combatBuffs.hadShieldBreak,
      hadComboBreaker: p.combatBuffs.hadComboBreaker,
      prestigeCount: this.prestigeCount,
      ...
      hadVillageTraining: p.combatBuffs.hadVillageTraining,
      consecutiveDeaths: this.consecutiveDeaths,
      consecutiveCrits: this.consecutiveCrits,
      isDangerZone: p.isDangerZone,
      comboStreak: this.comboStreak,
      dangerChainCount: this.dangerChainCount,
      hadBossFury: p.combatBuffs.hadBossFury,
      consecutiveBossKills: this.consecutiveBossKills,
      heroHp: p.hero.hp,
      heroHpMax: p.hero.hpMax,
      uniqueBossKills: this.uniqueBossKills,
      hadPrestigeSurge: p.combatBuffs.hadPrestigeSurge,
      hadVillageRestAtk: p.combatBuffs.hadVillageRestAtk,
      hadWaveMomentum: p.combatBuffs.hadWaveMomentum,
      hadRevengeStreak: p.combatBuffs.hadRevengeStreak,
      revengeStreakPower: this.revengeStreakPower,
      consecutiveWaveClears: this.consecutiveWaveClears,
      totalWins: this.totalWins,
      totalFights: this.totalWins + this.totalDeaths,
      hadEliteChainAtk: p.combatBuffs.hadEliteChainAtk,
      eliteCombo: this.eliteCombo,
      hadDeathAtkSurge: p.combatBuffs.hadDeathAtkSurge,
      dangerFights: this.dangerFights,
      hadVillageAtkTraining: p.combatBuffs.hadVillageAtkTraining,
      hadPrestigeEcho: p.hadPrestigeEcho,
      prestigeEchoDecay: p.prestigeEchoDecay,
      hadWaveExhaustion: p.hadWaveExhaustion,
      isElite: p.isElite,
      hpBelowBloodPact: p.hpBelowBloodPact,
      hpBelowAdrenalineRush: p.hero.hp < p.hero.hpMax * ADRENALINE_RUSH_HP_THRESHOLD,
      hadShieldSacrifice: p.hadShieldSacrifice,
      hadShieldBreakBurst: p.hadShieldBreakBurst,
      ...
      lowHpFury: p.lowHpFury,
      kind: p.kind,
      deathProximityCrit: this.deathProximityCrit,
      dangerStreak: this.dangerStreak,
      isPrestigeReady: p.hero.level >= this.getPrestigeThreshold(),
      activeSynergiesFromBloodFury: p.bloodFurySynergyActive,
      activeSynergiesFromElderWisdom: p.elderWisdomActive,
      activeSynergiesFromDesperateTrade: p.desperateTradeActive,
      antiSynergyActive: p.antiSynergyActive,
      goldenHourHighCombo: p.goldenHourHighCombo,
      wealthSacrificeActive: p.wealthSacrificeActive,
      synergyPrestigeBonus: this.synergyPrestigeBonus,
      hasEmberCrown: this.hasRelic(0),
      emberCrownStacks: this.emberCrownStacks,
      hasScholarLens: this.hasRelic(5),
      cursedAltarAtkBuff: this.cursedAltarAtkBuff,
      eventMomentumAtkActive: p.eventMomentumAtkActive,
      inspirationActive: p.inspirationActive,
      comboPrestigeFlat: p.comboPrestigeFlat,
      comboMilestoneBonus: this.comboMilestoneBonus,
      combatMastery: p.combatMasteryVal,
      waveChainAtk: p.waveChainAtk,
      deathCountAtk: p.deathCountAtk,
      ...
      heroAtk: computeBuffedHeroAtk(p.hero.atk, {
        ...
        earlyMomentumMul: 1 + EARLY_MOMENTUM_ATK_MUL,
        reputationMul: 1 + REPUTATION_AGG_ATK_MUL,
         ...
         * (this.midGameBuffs.isActive('ej_atk')
           ...
           : 1),
      // AtkMultiplierCalc only has attack-specific branches for these three
      // weather states. Normal/storm/snow are represented as null here; their
      // other effects are applied by their dedicated weather systems.
      weather: p.weather === 'rain' || p.weather === 'wind' || p.weather === 'fog'
        ? p.weather
        : null,
  ...
  private resolvePostCombatEvents(hero: HeroEntity, events: OverworldEvent[], isElite: boolean, isBoss: boolean): void {
    ...
    if (!r.eventType && shouldForceRareEvent(this.declineStack)) {
      const lateEvents: EventId[] = ['titan_arena', 'crimson_tithe', 'gold_crucible', 'astral_paradox', 'soul_forge', 'ascension_trial', 'echo_memory', 'shard_fusion'];
  ...
  private applyPendingEvents(r: PostCombatResult, hero: HeroEntity, events: OverworldEvent[]): void {
    ...
    const wanderingSageResolved = this.wanderingSageChoiceResolved;
    this.wanderingSageChoiceResolved = null;
    const eldersJudgmentResolved = this.eldersJudgmentChoiceResolved;
    this.eldersJudgmentChoiceResolved = null;
    const veteransChallengeResolved = this.veteransChallengeChoiceResolved;
    this.veteransChallengeChoiceResolved = undefined;
    const midGameResult = resolveMidGameEvents(
      ...
      },
      {
        ...
        reputationStyle: this.choiceHistory.getDominantStyle(),
        reputationTotalChoices: this.choiceHistory.totalChoices,
        reputationFired: this.reputationFired,
        veteransTrialFired: this.veteransTrialFired,
        finalReckoningFired: this.finalReckoningFired,
        ...
        wanderingSagePending: this.wanderingSagePending,
        wanderingSageChoiceResolved: wanderingSageResolved ?? undefined,
        eldersJudgmentFired: this.eldersJudgmentFired,
        eldersJudgmentPending: this.eldersJudgmentPending,
        eldersJudgmentChoiceResolved: eldersJudgmentResolved ?? undefined,
        veteransChallengeChoiceResolved: veteransChallengeResolved,
        veteransChallengeFired: this.veteransChallengeFired,
      },
    );
    ...
    if (midGameResult.provingPending) {
      ...
      events.push(...midGameResult.events);
    } else if (midGameResult.lastStandChoicePending) {
      ...
      events.push(...midGameResult.events);
    } else if (midGameResult.wanderingSageChoicePending) {
      // C921: Wandering Sage uses own pending mechanism
      this.wanderingSagePending = true;
      events.push({ type: 'wandering_sage_choice' });
      events.push(...midGameResult.events);
    } else if (midGameResult.eldersJudgmentChoicePending) {
      // C926: Elder's Judgment uses own pending mechanism
      this.eldersJudgmentPending = true;
      events.push({ type: 'elders_judgment_choice' });
      events.push(...midGameResult.events);
    } else if (midGameResult.veteransChallengePending) {
      // C959: Veteran's Challenge uses choiceEngine
      this.choiceEngine.triggerVeteransChallenge();
      this.veteransChallengeFired = true;
      events.push({ type: 'veterans_challenge_choice' });
      events.push(...midGameResult.events);
    } else {
    ...
    if (r.eventType && !r.merchantPending && !r.gamblerPending && !r.altarPending && !r.riskGambitPending && !r.wanderingMerchantPending && !r.sparringGroundsPending && !r.mercenaryOfferPending && !r.crossroadsPending) {
      if (isPostCombatSimpleEventType(r.eventType)) {
        events.push(POST_COMBAT_SIMPLE_EVENT_CREATORS[r.eventType]());
      }
    }
    if (r.eventChainReward) events.push({ type: 'event_chain_reward' });
  }
  ...
  private applyMidGameBuffs(result: import('./encounter/MidGameEventResolver').MidGameResult): void {
    ...
    // C959: Veteran's Challenge buff application
    if (b.veteransChallengeExpRemaining !== undefined) this.midGameBuffs.activate('vc_exp', b.veteransChallengeExpRemaining);
    if (b.veteransChallengeAtkRemaining !== undefined) this.midGameBuffs.activate('vc_atk', b.veteransChallengeAtkRemaining);
  ...
  private computeBuffAtkMultiplier(weatherSpeedMul: number): number {
    ...
    const inflBurst = this.midGameBuffs.isActive('inflation_burst') ? INFLATION_BURST_ATK_MUL : 1;
    return weatherSpeedMul * snowDrift * merchant * endgameSurge * echoMemory * vc * inflBurst;
  }
