...
export class EncounterEngine {
  ...
  private buildAtkContext(p: {
    ...
    return {
      ...
      heroAtk: this.stormNexusRemaining > 0 ? Math.floor(p.hero.atk * STORM_NEXUS_ATK_MUL) : p.hero.atk,
      weather: p.weather,
  ...
  private applyPendingEvents(r: PostCombatResult, hero: HeroEntity, events: OverworldEvent[]): void {
    ...
    if (r.eventType && !r.merchantPending && !r.gamblerPending && !r.altarPending && !r.riskGambitPending && !r.wanderingMerchantPending && !r.sparringGroundsPending && !r.mercenaryOfferPending) {
