...
export class EncounterEngine {
  ...
  resolveEncounter(hero: HeroEntity, kind: LandmarkKind, landmarkId: string): OverworldEvent[] {
    ...
    if (kind === 'enemy' || kind === 'boss') {
      ...
      const totalAtkMuls = Math.min(this.getAtkCap(), coreMuls * conditionMuls * goldMuls * combatMuls * progressMuls * chainMuls * tradeoffMuls * systemMuls);
      ...
      // C614: Group EXP multipliers into named categories for readability
      ...
      // C601: Group gold multipliers into named categories for readability
  ...
  private resolvePostCombatEvents(hero: HeroEntity, events: OverworldEvent[], isElite: boolean, isBoss: boolean): void {
    ...
    if (eventsEnabled && !eventTriggered && this.rng.chance(TREASURE_SHRINE_CHANCE)) {
      if (!this.choiceEngine.hasPendingShrineChoice()) {
