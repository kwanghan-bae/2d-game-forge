...
export class EncounterEngine {
  ...
  private titanArenaRemaining = 0; // C797: Titan Arena (enemy HP×1.5, ATK×1.3, EXP×2.0)
  private goldCrucibleRemaining = 0; // C800: Gold Crucible (ATK flat +50 for 15 fights)
  private goldCrucibleAtkFlat = 0; // C800: stored ATK bonus from gold burn
  ...
  private applyEventResolve(id: 'colosseum' | 'trial_grounds' | 'storm_nexus' | 'rain_sanctuary' | 'fog_ambush' | 'wind_gale' | 'snow_drift' | 'void_rift' | 'abyssal_convergence' | 'temporal_fissure' | 'titan_arena' | 'gold_crucible' | 'astral_paradox', accept: boolean): void {
    ...
    if (accept) {
      ...
      if (effects.goldCrucibleRemaining) {
        ...
        this.goldCrucibleAtkFlat = GOLD_CRUCIBLE_ATK_FLAT;
      ...
      while (eHp > 0 && !hero.staggered && hitCount < MAX_COMBAT_TURNS) {
        ...
        if (eHp > 0) {
          ...
          const totalDrMul = computeDamageReduction({
            ...
            titanArenaActive: this.titanArenaRemaining > 0, astralParadoxActive: this.astralParadoxRemaining > 0,
          });
