...
export function getLateGameDensityMul(totalFights: number): number {
  // C874: steeper slope 0.006→0.008 (fills mid-game faster)
  if (totalFights <= 120) return 1.0;
  if (totalFights <= 200) return 1.0 + (totalFights - 120) * 0.008; // →1.64 at 200
  if (totalFights <= 250) return 1.64 + (totalFights - 200) * 0.0064; // →1.96 at 250
  if (totalFights <= 350) return 1.96 + (totalFights - 250) * 0.002; // →2.16 at 350
  return Math.min(2.5, 2.16 + (totalFights - 350) / 400); // →2.5 at 536
}
