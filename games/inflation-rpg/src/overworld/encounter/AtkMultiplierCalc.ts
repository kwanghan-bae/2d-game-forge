...
export function computeAtkMultipliers(ctx: AtkMultiplierContext): AtkMultiplierResult {
  ...
  const flatAtk = ctx.heroAtk + ctx.comboPrestigeFlat + ctx.comboMilestoneBonus + ctx.combatMastery + ctx.waveChainAtk + ctx.deathCountAtk + ctx.dangerComboAtk + ctx.comboAtkMilestone + (ctx.goldCrucibleAtkFlat ?? 0);
