...
- **격리된 이전 버전 호환 표면** — legacy entrypoint의 `'korea_inflation_rpg_save'`
  localStorage 키와 `com.korea.inflationrpg` Capacitor appId가 그대로 살아있다.
  현재 게임의 canonical 저장은 `'shin-ui-eternal-sponsor-save-v2'`이며, 이전 세대의
  저장·계측·보상 키는 `legacyCompatibility.ts`에만 두고 읽기 전용으로 격리한다.
  `SaveManager`를 `@forge/core`로 승격할 때도 이 호환 경계를 유지한다.
