...
**한 줄 요약**: 한국 테마 2D 모바일 게임을 찍어내는 pnpm + Turborepo 모노레포.
공통 로직과 콘텐츠를 공유하면서 각 게임은 독립 앱으로 배포한다.
...
- **재사용 최대화**: 엔진 부트스트랩, 세이브, i18n, HUD 기반, 입력, 모바일 빌드
  파이프라인, E2E 훅을 공통 패키지로. 한국 설화·인물 같은 테마 콘텐츠도 공유 가능.
...
- 3개 게임을 병렬 구현 안 한다. 순차: inflation-rpg 이식 → 게임 A → 게임 B.
