...
**핵심 카타르시스:** 누적된 메타가 base power 가 되어, 캐릭터 레벨이 매 사이클 1부터 시작해도 빠른 성장 곡선이 화면에 펼쳐진다. "내가 키운 빌드 덕에 1레벨 hero 가 더 강한 사냥터로 직행한다" — Vampire Survivors / 키우기류 retention loop.
...
**캐릭터 레벨 정책:** 사이클 내 1 → 30~50 의 빠른 성장. **사이클 종료 시 1 로 리셋** (사용자 결정 — "다시 1부터 시작해야지 그래야 제한된 BP 내에 엄청나게 성장해나가는게 계속 보이지"). 사이클 EXP transfer 정책은 [Q-A](#12-open-questions-잔존) 에서 phase plan 시 결정.

**영구 누적:** gold / 강화 재료 / 장비 / ascension XP / 유물 진행도 / mythic / 캐릭터 unlock / 스토리 / 퀘스트 진행.
...
- 신규 metric: cycle 평균 duration, floor 도달 분포, encounter diversity, rival 등장 빈도, BP burn rate, EXP curve, drop tier 분포
