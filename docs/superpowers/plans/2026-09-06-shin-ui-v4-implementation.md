...
- V4 단위/컴포넌트 테스트: 391개 파일, 3,256개 테스트 통과.
- V4 Chromium·iPhone 14 E2E: 20/20 통과.
...
1. invalid V4 save를 조용히 새 게임으로 덮어쓰지 않는 복구 상태와 사용자 안내를 보강한다.
2. 시설·장비·원정 결과 화면의 남은 하드코딩 문구를 정적 데이터 helper로 통일한다.
3. 오프라인·개입·원정 결과의 중복 정산 및 앱 재개 경계를 추가 검증한다.
