...
- V4 단위/컴포넌트 테스트: 393개 파일, 3,266개 테스트 통과.
...
1. 저장·작업·원정 재시작 시에도 ID가 충돌하지 않도록 생성 규칙과 복구 경계를 보강한다.
2. 오프라인·개입·원정 결과의 중복 정산 및 앱 재개 경계를 추가 검증한다.
3. 시설·장비·원정 결과 화면의 남은 하드코딩 문구와 빈 보상 상태를 UX 관점에서 정리한다.
