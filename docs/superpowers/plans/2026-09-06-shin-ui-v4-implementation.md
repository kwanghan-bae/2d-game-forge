...
- 전체 게임 단위/컴포넌트 테스트(V4 포함): 398개 파일, 3,449개 테스트 통과.
...
- 장시간 V2 baseline은 dev-only fast-forward 후 회복 상태에서 다음 실제 Phaser 도착을 자연사로 연결해 전투 사망·무료 회춘 경합을 제거했다. `98a002ad` 이후 Chromium 4.0초·iPhone14 5.2초에 통과했고, 현재 HEAD에서도 전체 E2E 44/44(Chromium 22/22, iPhone14 22/22, 5.0분)을 통과했다. root `pnpm test`(game 398개 파일·3,449개 테스트 포함), `pnpm typecheck`(5개 package), `pnpm lint`, `pnpm circular`, `pnpm build`(game/dev-shell 포함)도 성공했다.
